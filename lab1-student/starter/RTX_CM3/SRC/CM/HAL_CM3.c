...
__asm U32 rt_get_PSP (void) {
        MRS     R0,PSP
        BX      LR // basically a return statement
}
...
void rt_init_stack (P_TCB p_TCB, FUNCP task_body) {
  ...
  // R1 R2 R3 R4 R5 R6 R7 R8 R9 R10 R11 R12 SP LR PC xPSR
  for (i = 0; i < 14; i++) {
    stk[i] = 0;
  }
...
/*--------------------------- os_tsk_acount_get -------------------------------*/
/*void os_tsk_count_get(void) {
        U32 total_count = 0;
        /* Initial Task stack pointer. *//*
        U32 task_stack_pointer = p_TCB->tsk_stack;
        while { // have not iterated through all of p_TCB
                total_count += 1;
        }

	// get all Running tasks
	// get all not-running tasks
	// make sure there are no inactive tasks
        // Return total tasks
        printf("Number of active tasks in system is %d: \n", total_count)
        return 0;
}

typdef struct rl_task_info {
        U8 state; // Task state [INACTIVE, READY, RUNNING, WAIT_DLY, WAIT_SEM, WAIT_MUT]
...
/*OS_RESULT os_tsk_get (OS_TID task_id, RL_TASK_INFO *buffer) {
        ...
        bufferInspectorPointer = buffer.ptask;

        p_task_info->task_id = task_id;
        p_task_info->state = *buffer->state;
        p_task_info->prio = *buffer->prio;
        p_task_info->stack_usage = *buffer->stack_usage;
        p_task_info->ptask = *buffer->*ptask 
        return OS_R_OK;
}*/
